...
{
	m_CurrentLevelName = "Level" + std::to_string(m_CurrentLevel) + std::to_string(GameInfo::GetInstance().GetPlayerHealth());
	...
	m_InputManagers.emplace(std::make_pair("Level_0", nullptr));
	m_InputManagers.emplace(std::make_pair("Level_1", nullptr));
	m_CurrentActiveLevelInput = 0;
...
{
	dae::ServiceLocator::GetSceneManager().RemoveScene(m_CurrentLevelName);
...
std::shared_ptr<dae::GameObject> LevelManager::CreateEnterName(const std::string& sceneName)
{
	...
	dae::ServiceLocator::GetSceneManager().SetActiveScene(scene);
...
{
	const std::string& sceneName{ m_CurrentLevelName };

	// INPUT 
	m_InputManagers["Level_" + std::to_string(m_CurrentActiveLevelInput)].reset();
	++m_CurrentActiveLevelInput;
	m_CurrentActiveLevelInput %= 2;
	m_InputManagers["Level_" + std::to_string(m_CurrentActiveLevelInput)] = std::make_shared<dae::InputManager>();
	dae::ServiceLocator::RegisterInputManager(m_InputManagers["Level_" + std::to_string(m_CurrentActiveLevelInput)].get());

	auto pauseInput = dae::InputAction(0, dae::ButtonState::downThisFrame, std::make_shared<PauseCommand>(nullptr), dae::ControllerButton::Start, dae::KeyboardKey::K_Tab);
	...
	std::string startText = "Lives: " + std::to_string(GameInfo::GetInstance().GetPlayerHealth());
	...
	case LevelManager::GameMode::Coop:
	{
		...
		player->GetComponent<HealthComponent>()->AddObserver(healthDisplay_comp);
...
void LevelManager::StartGame()
{
	...
	GameInfo::GetInstance().SetPlayerHealth(3);
	...
	, int playerIdx, float movementSpeed)
{
	...
	dae::Scene* scene = dae::ServiceLocator::GetSceneManager().GetScene(m_CurrentLevelName);
	...
	// HEALTH
	player_go->AddComponent(std::make_shared<HealthComponent>(player_go, GameInfo::GetInstance().GetPlayerHealth(), 1.f));
